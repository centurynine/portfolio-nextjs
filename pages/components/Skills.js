        ...
        <div className="flex flex-col md:flex-row md:space-x-10">
          {skills.map((skillGroup, index) => (
            <div key={index} className="mb-8">
              <h2 className={`${darkMode ? 'text-gray-200' : 'text-slate-700'} text-3xl mb-4`}>
                {skillGroup.title}
              </h2>
              <div className="flex flex-row flex-wrap gap-4">
                {skillGroup.skills.slice(0, 2).map((skill, skillIndex) => (
                  <div
                    key={skillIndex}
                    className={`${darkMode ? 'bg-neutral-700 hover:bg-neutral-800' : 'bg-gray-100'} flex items-center gap-2 rounded-full p-2 hover:bg-slate-400 transition`}
                  >
                    <img
                      src={skill.image}
                      alt={skill.name}
